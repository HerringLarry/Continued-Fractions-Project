...
(define (addition cf-init a b)
  ...
  (define (overall tbt cf whole-fr num)
    (newline)
    (display 'tbt)
    (display tbt)
    (newline)
   ; (newline)
    (cond ((and (= (s-c tbt) 0) (= (s-d tbt) 0))
           '())
          ((or (= (s-c tbt) 0) (= (s-d tbt) 0))
           (overall (ingestion tbt (gen cf whole-fr)) (gen-rest-cf cf) (gen-rest-whole-fr cf whole-fr) 1))
          ((= (car (gen-rest-whole-fr cf whole-fr)) 0)
           (cons-stream (quotient (s-b tbt) (s-d tbt)) (overall (egestion tbt (quotient (s-a tbt) (s-c tbt))) cf whole-fr 2))) 
          ((= (quotient (s-a tbt) (s-c tbt)) (quotient (s-b tbt) (s-d tbt)))
           (cons-stream (quotient (s-a tbt) (s-c tbt)) (overall (egestion tbt (quotient (s-a tbt) (s-c tbt))) cf whole-fr 3)))
          (else (overall (ingestion tbt (gen cf whole-fr)) (gen-rest-cf cf) (gen-rest-whole-fr cf whole-fr) 4))))
  (overall '((1 2) (2 0)) cf-init (list b 0) 0))

(display-cf (addition (convert-fr-cf 13 11) 1 2) 100)
